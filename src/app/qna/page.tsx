...
export default function QnaPage() {
  ...
  return (
    <div className="min-h-screen bg-white pb-12">
      <header className="sticky top-0 right-0 px-4 py-4 flex items-center justify-end backdrop-blur-md z-20 w-full">
      ...
      <div className="container mx-auto px-4 pt-16 max-w-4xl">
        <h1 className="text-2xl sm:text-3xl font-bold text-[#0090FF] mb-3 sm:mb-4">Frequently Asked Questions</h1>
        ...
        <div className="space-y-3 sm:space-y-4">
          {faqs.map((faq) => (
            ...
            >
              <button
                className="w-full flex justify-between items-center p-3 sm:p-4 text-left bg-white hover:bg-gray-50 focus:outline-none"
                onClick={() => toggleFaq(faq.id)}
              >
                <span className="font-medium text-gray-900 text-sm sm:text-base">{faq.question}</span>
                <svg 
                  className={`w-4 h-4 sm:w-5 sm:h-5 text-gray-500 transition-transform ${openId === faq.id ? 'transform rotate-180' : ''}`} 
                  fill="none" 
                  viewBox="0 0 24 24" 
                  stroke="currentColor"
                >
        ...
        <div className="mt-10 sm:mt-16 bg-white p-4 sm:p-8 rounded-lg shadow-sm">
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-3 sm:mb-4">Have More Questions?</h2>
          ...
          <div className="flex flex-wrap gap-3 sm:gap-4">
            <Link 
              href="/docs" 
              className="bg-[#0090FF] text-white py-2 px-4 sm:px-6 rounded-md hover:bg-opacity-80 transition-colors text-sm sm:text-base"
            >
              Read the Docs
            </Link>
            <a 
              href="https://t.me/+MznFxMPcIhM3ZDI1" 
              className="bg-white text-[#0090FF] border border-[#0090FF] py-2 px-4 sm:px-6 rounded-md hover:bg-[#0090FF] hover:bg-opacity-10 transition-colors text-sm sm:text-base"
            >
              Join our Community
            </a>
          </div>
